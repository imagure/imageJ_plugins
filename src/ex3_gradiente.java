...
public class ex3_gradiente implements PlugInFilter {
	ImagePlus impf2;
			...
			for(int j1=b; j1< (h-b) ; j1++) {
				...
				for (int i2=-a; i2<=a; i2++) {
					for (int j2=-b; j2<=b; j2++) {
						dx = dx + x_kernel[i2+a][j2+b] * (double)ipf2.getf(i1-i2, j1-j2);
						dy = dy + y_kernel[i2+a][j2+b] * (double)ipf2.getf(i1-i2, j1-j2);
